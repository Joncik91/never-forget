...
export function evaluateGate(
  gate: LogicGate,
  project: Project,
): { triggered: boolean; action: LogicGate["action"] } {
  if (!gate.enabled) {
...
function resolveProjectProperty(
  property: string,
  project: Project,
): string | undefined {
  switch (property) {
...
function resolveDependencyProperty(
  property: string,
  dep: Project["dependencies"][number],
): string | undefined {
  switch (property) {
    case "state":
      return dep.state;
