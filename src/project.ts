...
export function updateDependencyState(
  project: Project,
  dependencyId: string,
...
export function addTask(project: Project, task: Task): Project {
  return touch({
    ...project,
...
export function transitionState(
  ...
  if (!allowed.includes(newState)) {
    throw new InvalidStateTransitionError(
      project.state,
      newState,
      `Valid transitions from ${project.state}: ${allowed.join(", ")}`,
    );
  }
...
export function evaluateEntryGate(project: Project): GateResult {
  ...
  for (const dep of project.dependencies) {
    if (dep.priority === "required") {
      if (dep.state === "undefined" || dep.state === "identified") {
        blockers.push(
          `Required dependency "${dep.name}" (${dep.id}) is not ready (state: ${dep.state})`,
        );
      ...
      if (project.requireProofOfWork && !dep.proofOfWork) {
        blockers.push(
          `Required dependency "${dep.name}" (${dep.id}) has no proof of work`,
        );
...
export function passEntryGate(project: Project): Project {
...
export function runPreFlightChecks(
  ...
  const requiredDeps = project.dependencies.filter(
    (d) => d.priority === "required",
  );
  checks.push({
  ...
  const unidentified = requiredDeps.filter(
    (d) => d.state === "undefined",
  );
  checks.push({
...
export function runPostFlightChecks(
  project: Project,
): FlightCheckResult {
  const checks: CheckResult[] = [];
  ...
  const unverified = requiredDeps.filter(
    (d) => d.state !== "verified" && d.state !== "complete",
  );
  checks.push({
  ...
  const incompleteReactions = allReactions.filter(
    (r) => r.state !== "complete",
  );
  checks.push({
  ...
  if (project.requireProofOfWork) {
    const missingProof = requiredDeps.filter(
      (d) => !d.proofOfWork,
    );
    checks.push({
